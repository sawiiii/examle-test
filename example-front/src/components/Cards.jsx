...
const Cards = () => {
  ...
  const callApi = () => {
    ...
    Axios.get(makeEndPoint("payments"))
      .then((response) => {
  ...
  const handleFilter = (e) => {
    ...
    // console.log(`Cookie :${Cookies.get('token')}`);
    const config2 = {
      params: { day: fields.day, month: fields.month, year: fields.year, type: fields.type },
    };
    console.log(config2);
    Axios.get(makeEndPoint("payments/filter"), config2)
      .then((response) => {
        const Data = response.data;
        setData(Data);
        setsales(Data.data);
    ...
    <div className="ml-auto mb-6 lg:w-[75%] xl:w-[80%] 2xl:w-[85%]">
          <div className="sticky z-10 top-0 h-16 border-b border-neutral-200 bg-white lg:py-2.5">
              <div className="px-6 flex items-center justify-between space-x-4 2xl:container">
                  <h5 hidden className="text-2xl text-gray-600 font-medium lg:block">Payment</h5>
              </div>
          </div>
          <div className=' m-4 p-4 rounded-lg'>
            <div className=' shadow-md p-2 w-auto flex flex-row items-center rounded-md'>
              ...
              <form onSubmit={handleFilter}>
                ...
                > 
                <option className="text-sm" value=''>Type</option>
                {data.dates.types.map((data, key) => {
                  return (
                ...
                <div className="flex flex-wrap items-center justify-center space-x-2">
                  <div className="w-1/3 py-8 px-6 space-y-6 rounded-xl border border-gray-200 bg-white shadow-md">
                            ...
                            <table className="w-full text-gray-600">
                                <tbody>
                                    <tr>
                                        <td className="py-2">Porcentage on Debit</td>
                                        <td className="text-gray-500">{data.porDeb} %</td>
                                    </tr>
                                    <tr>
                                        <td className="py-2">Porcentage on Credit</td>
                                        <td className="text-gray-500">{data.porCred} %</td>
                                    </tr>
                                    <tr>
                                        <td className="py-2">Porcentage on Cash</td>
                                        <td className="text-gray-500">{data.porCash} %</td>
                                    </tr>
                                </tbody>
                            </table> 
                  </div>
                  <div className="w-1/3 h-[290px] py-8 px-6 space-y-6 rounded-xl border border-gray-200 bg-white shadow-md overflow-scroll">
                            ...
                            <table className="w-full text-gray-600">
                                <tbody>
                                {data.profitTypes.map((data, key) => {
                                    return (
                                      <tr key={key}>
                                        <td className="py-2">{data.val}</td>
                                        <td className="text-gray-500">$ {data.earn}</td>  
                                      </tr>
                                    );
                                })}
